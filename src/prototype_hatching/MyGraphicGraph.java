...
public class MyGraphicGraph {
	...
	private double angle; //ハッチング の角度
	private double kankaku = 15; //ハッチング の線同士の間隔

	public MyGraphicGraph(Graphics g) {
		my_g = g;
		setAngle(3.1415926535/4);
		//setAngle(3.1415926535/1000);
	}
	...
	//斜線(左上から右肩下がり)
		public void polygonHatching0(Polygon p) {
			int i;

			// 枠を描画する
			my_g.drawPolygon(p);
			my_g.drawLine(p.xpoints[0] , p.ypoints[0] , p.xpoints[p.npoints - 1] , p.ypoints[p.npoints - 1]);
...
//			if (Math.abs(dbAngleX) >= Math.abs(dbAngleY))	// 横線に近い線
			{
				...
				while (y <= rectBounding.y + rectBounding.height - dbAngle * rectBounding.x)
				{
					...
					if (!stackPoint.empty() && (nStackLen > 1))
					{
						...
						{
							my_g.drawLine(pt[i].x , pt[i].y , pt[i + 1].x , pt[i + 1].y);
						}
					}
					y += nGapY;
				}
	...
	//斜線(左上から右肩下がり)
	public void polygonHatching1(Polygon p) {
		int i;
...
//		if (Math.abs(dbAngleX) >= Math.abs(dbAngleY))	// 横線に近い線
	...
	//縦線
	public void polygonHatching2(Polygon p) {
		int i;
...
//		if (Math.abs(dbAngleX) >= Math.abs(dbAngleY))	// 横線に近い線
	...
	//塗りつぶし
	public void polygonHatching3(Polygon p) {
		int i;

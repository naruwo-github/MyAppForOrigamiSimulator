...
public class MyBezier {
	...
	public static void calcBezierWidCol(List<Integer> cpx, List<Integer> cpy, Graphics g, List<Color> color, List<Integer> wid, Graphics2D g2) {
		if(cpx.size() % 4 == 0) {
			int j = 0;
		    for(int i = 0; i < cpx.size(); i+=4) {
		    	BasicStroke bs2 = new BasicStroke(wid.get(j), BasicStroke.CAP_ROUND, BasicStroke.JOIN_ROUND);
				g2.setStroke(bs2);
				g.setColor(color.get(j));
		    	drawBezier(g,cpx.get(i),cpy.get(i),cpx.get(i+1),cpy.get(i+1),cpx.get(i+2),cpy.get(i+2),cpx.get(i+3),cpy.get(i+3));
		    	j++;
		    }
		  }

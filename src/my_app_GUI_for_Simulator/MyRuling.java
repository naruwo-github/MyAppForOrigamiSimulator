...
public class MyRuling {
	...
	//heiaktsu
		...
		static void drawRuling2(Color color, Graphics g, List<Integer> cpx, List<Integer> cpy, List<Integer> coorinx, List<Integer> cooriny, List<Double> heikatsu, List<Integer> rux, List<Integer> ruy, List<List<Integer>> orix, List<List<Integer>> oriy) {
			...
			for(int i = 1; i < (cpx.size()/4-1)/2; i++) {
				...
				calcRuling2(color,g,rx,ry,pre_cx,pre_cy,post_cx,post_cy,cpx.get((i+1)*8+0),cpy.get((i+1)*8+0),cpx.get((i+1)*8+1),cpy.get((i+1)*8+1),cpx.get((i+1)*8+2),cpy.get((i+1)*8+2),cpx.get((i+1)*8+3),cpy.get((i+1)*8+3),coorinx,cooriny,rux,ruy,orix,oriy);
				pre_cx = rx;
				pre_cy = ry;
			}
	...
	//heiaktsu
	//rulingスタートは第三入力線(第一補助線)上を分割した点
	static void drawRuling2(Color color, Graphics g, List<Integer> cpx, List<Integer> cpy, List<Integer> coorinx, List<Integer> cooriny, List<Double> heikatsu, List<Integer> rux, List<Integer> ruy, List<Integer> orix, List<Integer> oriy) {
		start_x = new ArrayList<Integer>();
		start_y = new ArrayList<Integer>();
		pre_cx = new ArrayList<Integer>();
		pre_cy = new ArrayList<Integer>();

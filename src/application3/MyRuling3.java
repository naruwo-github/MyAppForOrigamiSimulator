...
public class MyRuling3 {
	...
	static void drawRuling2(Graphics g, List<Integer> cpx, List<Integer> cpy, List<Integer> coorinx, List<Integer> cooriny, List<Double> heikatsu, List<Integer> rux, List<Integer> ruy, List<Integer> orix, List<Integer> oriy) {
		...
		MyBezier3.beziPoint(pre_cx,pre_cy,cpx.get(0),cpy.get(0),cpx.get(1),cpy.get(1),cpx.get(2),cpy.get(2),cpx.get(3),cpy.get(3));
		MyBezier3.beziPoint(post_cx,post_cy,cpx.get(4),cpy.get(4),cpx.get(5),cpy.get(5),cpx.get(6),cpy.get(6),cpx.get(7),cpy.get(7));
		...
		for(int i = 0; i < heikatsu.size(); i++) {
		    ...
		    //float x1 = x0;
		    //float y1 = y0;
		    MyVector2d3 vec0 = new MyVector2d3(x0, y0);
		    ...
		    MyVector2d3 svec = v1.sub(v0);
		    MyVector2d3 hvec = new MyVector2d3(svec.y, -svec.x);//これは第二入力線方向
		    MyVector2d3 hvec1 = new MyVector2d3(-hvec.x, -hvec.y);
		    svec.normalize();
		    hvec.normalize();
		    hvec1.normalize();
		    ...
		    for(j = 0; j < pre_cx.size(); j++) {
		    	MyVector2d3 vec = new MyVector2d3(pre_cx.get(j),pre_cy.get(j));
		    	MyVector2d3 direc = vec.sub(vec0);
		    	direc.normalize();
		    ...
		    for(k = 0; k < post_cx.size(); k++) {
		    	MyVector2d3 vec = new MyVector2d3(post_cx.get(k),post_cy.get(k));
		    	MyVector2d3 direc = vec.sub(vec0);
		    	direc.normalize();
		    ...
		    satmp = 0.0;
		    int l;
		    //for(l = 0; l < coorinx.size(); l++) {
		    for(l = coorinx.size() - 2000; l < coorinx.size(); l++) {
		    	MyVector2d3 vec = new MyVector2d3(coorinx.get(l),cooriny.get(l));
		    	MyVector2d3 direc = vec.sub(vec0);
		    	direc.normalize();
		    	satmp = Math.sqrt((direc.x - hvec.x)*(direc.x - hvec.x)+(direc.y - hvec.y)*(direc.y - hvec.y));
		    	if(sa1 > satmp) {
		...
		*/
		...
		for(int i = 1; i < (cpx.size()/4-1)/2; i++) {
			...
			MyBezier3.beziPoint(post_cx,post_cy,cpx.get((i+1)*8-4),cpy.get((i+1)*8-4),cpx.get((i+1)*8-3),cpy.get((i+1)*8-3),cpx.get((i+1)*8-2),cpy.get((i+1)*8-2),cpx.get((i+1)*8-1),cpy.get((i+1)*8-1));

...
public class MyRuling {
	...
	//test
	static void drawRulingProto(Graphics g, List<Integer> cpx, List<Integer> cpy, List<Double> heikatsu) {
		start_x = new ArrayList<Integer>(); //rulingスタートの点
		start_y = new ArrayList<Integer>();
		pre_cx = new ArrayList<Integer>(); //描画元曲線
		pre_cy = new ArrayList<Integer>();
		post_cx = new ArrayList<Integer>(); //描画先曲線
		post_cy = new ArrayList<Integer>();
		...
		List<Integer> rinkakux = new ArrayList<Integer>(); //輪郭を格納するやーつ
		List<Integer> rinkakuy = new ArrayList<Integer>();

		//輪郭を保存したやつ
		MyVector2d ex1 = new MyVector2d(cpx.get(0),cpy.get(0));
		MyVector2d ex2 = new MyVector2d(cpx.get(4),cpy.get(4));
		MyVector2d ex3 = ex2.sub(ex1);
		ex3.div(1000);
		MyVector2d ex4 = new MyVector2d(cpx.get(3),cpy.get(3));
		MyVector2d ex5 = new MyVector2d(cpx.get(7),cpy.get(7));
		MyVector2d ex6 = ex5.sub(ex4);
		ex6.div(1000);
		double x = ex1.x;
		double y = ex1.y;
		double x2 = ex4.x;
		...
		while(i < start_x.size()) {
			...
			for(t = 0; t < 1; t+=0.001) {
			    ...
			    MyVector2d v1 = new MyVector2d(vx1, vy1);
			    //tの時のベジェの接ベクトル
			    MyVector2d sessenvec = v1.sub(v0);
			    MyVector2d hosenvec1 = new MyVector2d(sessenvec.y, -sessenvec.x); //start方向
			    MyVector2d hosenvec2 = new MyVector2d(-hosenvec1.x, -hosenvec1.y);//post方向
			    //正規化
			    sessenvec.normalize();
			    hosenvec1.normalize();
			    hosenvec2.normalize();
			    //スタート点
			    MyVector2d start = new MyVector2d(start_x.get(i), start_y.get(i));
			    //スタート点から、制御線方向のベクトル
			    MyVector2d startToBezi = vec0.sub(start);
			    startToBezi.normalize();
			    double tmpnaiseki = sessenvec.naiseki(startToBezi);
			    if(tmpnaiseki < 0.00000000001 && a == 0) {
				    x0 = (float) (Math.pow((1-t),3)*cpx.get(8)+3*t*Math.pow((1-t),2)*cpx.get(9)+3*(1-t)*Math.pow(t,2)*cpx.get(10)+Math.pow(t,3)*cpx.get(11));
				    y0 = (float) (Math.pow((1-t),3)*cpy.get(8)+3*t*Math.pow((1-t),2)*cpy.get(9)+3*(1-t)*Math.pow(t,2)*cpy.get(10)+Math.pow(t,3)*cpy.get(11));
				    g.setColor(Color.GREEN);
				    g.drawOval((int)x0, (int)y0, 3, 3);
				    g.setColor(Color.RED);
			    	g.drawLine(start_x.get(i), start_y.get(i), (int)x0, (int)y0);
			    	t+=0.1;
			    	a++;
			    }
			}
